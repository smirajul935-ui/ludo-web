...
// Aapka bheja hua PATH aur Coordinates
...
function updateUI() {
    ...
    // Dice enable/disable logic
    const rollBtn = document.getElementById('roll-btn');
    if(roomData.turn === myColor && !roomData.diceRolled) {
        rollBtn.disabled = false;
        rollBtn.style.boxShadow = "0 0 15px #29ff29";
    } else {
...
async function rollDice() {
    ...
    let anim = setInterval(async () => {
        ...
        if(count++ > 8) {
            ...
            // Auto switch turn if no moves
            let hasMove = roomData.tokens[myColor].some(p => isValidMove(p, diceVal));
            if(!hasMove) setTimeout(() => switchTurn(false), 1000);
        }
    }, 50);
}
